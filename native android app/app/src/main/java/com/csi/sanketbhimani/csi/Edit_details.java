...
public class Edit_details extends AppCompatActivity implements View.OnClickListener {
    ...
    public void onClick(View V){
        if(V.getId() == R.id.email_sign_up_button){
            ...
            userSignup = new UserEdit(name,email,year,branch,password,a,this);
...
class UserEdit extends AsyncTask<Void, Void, String>{

    Edit_details sa_class;
    String name;
    String email;
    String password;
    String year;
    String branch;
    String csi_id;
    UserEdit(String n, String e, String y, String b, String p,String id, Edit_details sa){
...
csi_id=id;
    ...
    protected String doInBackground(Void... params) {
        ...
        String user_id = sharedpreferences.getString("user_id","not_registered");
        ...
        if(sa_class.a.equals("yes")) {
            try {
                httpget= new HttpGet("http://192.168.43.190/csi_data/CSI_ANDROID_APP_SERVER/editUser.php?is_csi_member=" + "yes" + "&csi_id="+csi_id+"&email=" + URLEncoder.encode(email, "UTF-8") + "&password=" + URLEncoder.encode(password, "UTF-8") + "&name=" + URLEncoder.encode(name, "UTF-8") + "&year=" + URLEncoder.encode(year, "UTF-8") + "&branch=" + URLEncoder.encode(branch, "UTF-8")+"&userid="+user_id);
            } catch (UnsupportedEncodingException e) {
                e.printStackTrace();
            }
        }else if(sa_class.a.equals("no")){
            httpget= new HttpGet("http://192.168.43.190/csi_data/CSI_ANDROID_APP_SERVER/editUser.php?email="+email+"&password="+password+"&name="+name+"&year="+year+"&branch="+branch+"&userid="+user_id);

        }
        try {
            Log.e("###","http://192.168.43.190/csi_data/CSI_ANDROID_APP_SERVER/editUser.php?is_csi_member=" + "no" + "&email=" + URLEncoder.encode(email, "UTF-8") + "&password=" + URLEncoder.encode(password, "UTF-8") + "&name=" + URLEncoder.encode(name, "UTF-8") + "&year=" + URLEncoder.encode(year, "UTF-8") + "&branch=" + URLEncoder.encode(branch, "UTF-8")+"&userid="+user_id);
        } catch (UnsupportedEncodingException e) {
            e.printStackTrace();
        }
